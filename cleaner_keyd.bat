@echo off
chcp 65001
title Magic's FiveM Tool
cls
mode 80,15
echo.
echo.
echo   LOADING FIVEM SPOOFY WOOFR...
timeout 3 >nul
echo.
echo.
echo [32m[] Activated Base[0m
echo.
echo [32m[] Searching For Offsets[0m
timeout 2 >nul
echo.
...
cls
color 05
TITLE LOGIN PAGE (MAGIC SPOOFER)
set /p user=Username :
if %user%==NXT-G3N-MAGIC-SPOOFER-PERMA-KEY (
goto LOGIN2
)
if %user%==Magical-spoof (
goto LOGIN2
)
echo WRONG. CLOSING PROGRAM IN 3 SECONDS
timeout 3 >nul
goto END
...
echo.
set /p pass=Password :
if %pass%==NXT-MAGIC-SPOOFER-PERMA-KEY (
goto :MENU
)
if %pass%==MY (
goto :MENU
...
:FIXDISCORD
cls
RENAME %userprofile%\AppData\Roaming\discord\0.0.309\modules\MAGICAL_SPOOFER discord_rpc
...
RENAME %userprofile%\AppData\Roaming\discord\0.0.309\modules\discord_rpc MAGICAL_SPOOFER
timeout 1 >nul
cls
echo.
echo.
